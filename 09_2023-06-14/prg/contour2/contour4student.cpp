...
//
//
//  Created by Masashi Morimoto on 2023/06/06.
//
...
int main(int argc, const char * argv[]) {
    ...
    //8.2 表示する輪郭番号の入力
    int contour_num;
    printf("表示する輪郭番号を入力してください (0〜%d): ", (int)contours.size()-1);
    scanf("%d", &contour_num);

    //8.3 領域特徴量の取得・表示
    double L, S;
    L = cv::arcLength(contours[contour_num], true);
    S = cv::contourArea(contours[contour_num]);
    printf("周囲長=%f, 面積=%f\n", L, S);
    ...
    //8.5 外接長方形の描画
    cv::rectangle(dst_img, rect, CV_RGB(255, 0, 255), 3);
